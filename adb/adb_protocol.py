    ...
    def Connect(cls, usb, banner=b'notadb', rsa_keys=None, auth_timeout_ms=100):
        ...
        if cmd == b'AUTH':
            ...
            for rsa_key in rsa_keys:
                ...
                # MODIFIED: Do not mangle the banner property here by converting it to a string
    ...
    def InteractiveShellCommand(cls, connection, command=None, strip_command=True, delimiter=None, strip_delimiter=True,
        ...
        try:
            ...
            if clean_stdout:
                ...
                # first_backspace_pos = stdout_bytes.find(b'\x08')
                # if first_backspace_pos == -1:
                #     cleaned_stdout_stream.write(stdout_bytes)
                # else:
                #     end_backspaces_pos = first_backspace_pos
                #     while True:
                #         if chr(stdout_bytes[end_backspaces_pos]) == '\b':
                #             end_backspaces_pos += 1
                #         else:
                #             break
                #
                #     num_backspaces = end_backspaces_pos - first_backspace_pos
                #
                #     # Ignore num_backspaces number of characters prior to first_backspace_pos
                #     cleaned_stdout_stream.write(stdout_bytes[:(first_backspace_pos-num_backspaces)])
                #     cleaned_stdout_stream.write(stdout_bytes[end_backspaces_pos:])
            ...
            if delimiter and strip_delimiter:
                # Sometimes, a number preceeds the terminal delimiter which we need to check for an strip out
                # eg. "123|dreamlte:/ $"

                # number_delim = r'(?P<new_delim>\d{1,3}\|' + delimiter.decode('utf-8') + ")"
                #
                # reg = re.compile(number_delim)
                # m = reg.match(stdout.decode('utf-8'))
                # if m:
                #     stdout = stdout.replace(m.group('new_delim'), b'')
                # else:
